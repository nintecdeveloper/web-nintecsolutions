...
root=Path(__file__).resolve().parents[1];dist=root/'dist';errors=[];pages=list(dist.glob('*/index.html'))+[dist/'index.html']
...
for path in pages:
 ...
 if p.h1!=1 or p.title!=1 or p.lang!='ca' or p.canonical!=1 or p.description!=1:errors.append(f'{path}: metadata/headings')
...
assert len(pages)==10
assert not errors,'\n'.join(errors)
print(f'PASS: {len(pages)} pages; internal links/anchors/assets; one h1/title/canonical/description per page; lang=ca; form labels; unique IDs; valid sitemap XML.')
